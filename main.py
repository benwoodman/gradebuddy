...
DATABASE_TABLE = os.environ['DB_TABLE']

DO_S3_ACCOUNT = os.environ['S3_ACCOUNT']
DO_S3_SECRET = os.environ['S3_SECRET']
...
def get_user(username):
    # username = 'babin101'
    connection = create_connection(DATABASE_URL, DATABASE_NAME, DATABASE_PASSWORD, DATABASE_TABLE)
...
async def get_upload_information(upload_id: int, current_user: User = Depends(get_current_user)):
    ...
    client = s3_session.client('s3', region_name='nyc3', endpoint_url='https://nyc3.digitaloceanspaces.com', aws_access_key_id=DO_S3_ACCOUNT, aws_secret_access_key=DO_S3_SECRET)
